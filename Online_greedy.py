...
#            disp_payperiod.loc[disp_payperiod['j'] == ventana, 'd_j'] = disp_payperiod.loc[disp_payperiod['j'] == ventana, 'd_j']+duracion_serv #hora de disponibilidad vehiculo asignado
        ...
        if len(veh_disp) == 1:
            ...
            for k in range(len(veh_disp)):
                ...
                auto = veh_disp.iloc[k,1]
                # tiempo_aux = disp_payperiod.loc[disp_payperiod['m_j'] == auto,'e_j'].sum()
                ventana = veh_disp.iloc[k,0] #ventana disponible
                tiempo_aux = disp_payperiod.loc[disp_payperiod['m_j'].eq(auto) & disp_payperiod['j'].ge(ventana),'e_j'].sum()
                actual = valor/tiempo_aux
                veh_aux.loc[veh_aux['id_m'] == auto, 'B_m'] = (veh_aux.loc[veh_aux['id_m'] == auto, 'B_m'].values+actual) #entrega ratio de ingreso-tiempo
                ratio = pd.concat([ratio, veh_aux['B_m']], axis=1,)
                varianza_veh = ratio.var(ddof=0)
                varianza_veh.index = range(varianza_veh.shape[0])
            
            #Selecciona el mejor caso (menor varianza producida)
            asig = varianza_veh.idxmin()

            vehiculo_aux.append(asig) # indica que vehiculo hizo tal servicio
            
            plata = plata + valor # indicador de dinero recaudado        
            
            #reemplaza la varianza
            veh['B_m'] = ratio.iloc[:,asig]
            
            #actualiza ventanas de trabajo vehiculo asignado
            #vehiculo = veh_disp.iloc[asig,1]
            ventana = veh_disp.iloc[asig,0]
            disp_payperiod.loc[disp_payperiod['j'] == ventana, 'd_j'] = disp_payperiod.loc[disp_payperiod['j'] == ventana, 'd_j']+duracion_serv #hora de disponibilidad vehiculo asignado
